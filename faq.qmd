...
Yes, this is possible, although the possibilities differ a bit depending on the software you are using and the type of moderator. Moderation by a time-invariant, categorical variable can be investigated using a multiple group RI-CLPM, as described in Extension 2 of @mulder_three_2021. Moderation by a time-varying, continuous variable can be investigating by creating latent interactions of the moderator $\times$ the within-components and/or random intercepts. See @ozkok_interaction_2022 and @speyer_testing_2023 for detailed information on how to this. 

@ozkok_interaction_2022 addresses the high computational burden that the inclusion of a continuous time-varying moderator places on the estimation, and they make use of a Bayesian estimator. Their supplemental materials, including their Mplus model syntax, can be found at [https://journals.sagepub.com/doi/suppl/10.1177/10944281211043733](https://journals.sagepub.com/doi/suppl/10.1177/10944281211043733). They note that currently only Mplus supports the use of latent interactions as used here. 
:::
...
`powRICLPM` is an R-package that performs a power analysis for the RI-CLPM in a simple and user-friendly way. It implements the strategy as proposed by @mulder_power_2022, and includes various extensions such as the inclusion of measurement error (leading to the stable trait autoregressive trait state model by @collins_traitstate_2001, and multiple constraints over time. `powRICLPM` is available from the Comprehensive R Archive Network (CRAN). More information can be found at [https://jeroendmulder.github.io/powRICLPM/](https://jeroendmulder.github.io/powRICLPM/).
:::
