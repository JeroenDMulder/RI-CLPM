...
## Alternative syntax {.tabset .tabset-fade}

The basic RI-CLPM model syntax (with and without constraints) can be made considerably simpler by using the `^`-notation and pairwise `ON` and `WITH` statements: 
...
### The basic model
    ...
    MODEL:      ! Create two individual factors (random intercepts)
                ...
                ! Using the residual-language ("^"-notation) Mplus automatically
                ! creates within-person centered variables with measurement 
                ! error variances constrained at 0. 
    
                ! Estimate the lagged effects between within-person centered 
                ! variables using "Pairwise ON"-notation.
                x2^-x5^ y2^-y5^ PON x1^-x4^ y1^-y4^; ! Autoregressive effects
                x2^-x5^ y2^-y5^ PON y1^-y4^ x1^-x4^; ! Cross-lagged effects
        
                ! Estimate covariance between the within-person components 
                ! using "Pairwise WITH"-notation.
                x1^-x5^ PWITH y1^-y5^;
...
### Constraints over time
    ...
    MODEL:      ! Create two individual factors (random intercepts)
                ...
                ! Estimate (residual) (co)variance between the within-person 
                ! components using "Pairwise WITH"-notation.
